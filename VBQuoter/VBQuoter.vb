...
Public Class VBQuoter
  ...
  Private Overloads Function Quote(treeElement As Object, Optional name As String = Nothing) As ApiCall
    ...
    If (TypeOf treeElement Is SyntaxNodeOrToken) Then
      ...
      Return If(syntaxNodeOrToken.IsNode, Quote_SyntaxNode(syntaxNodeOrToken.AsNode(), name), QuoteToken(syntaxNodeOrToken.AsToken(), name))
    End If
    Return Quote_SyntaxNode(DirectCast(treeElement, SyntaxNode), name)
  End Function
  ...
  ''' </summary>
  Private Function Quote_SyntaxNode(node As SyntaxNode, name As String) As ApiCall
    Select Case node
      ...
      Case asClause          As AsClauseSyntax                   : Quote_AsClause(asClause)
      ...
      Case constraint        As ConstraintSyntax                  : Quote_ConstrainClause(constraint)
      ...
      Case field             As FieldInitializerSyntax             : Quote_FieldInitializerSyntax (field)
      ...
      Case importsClause     As ImportsClauseSyntax                 : Quote_ImportsClauseSyntax(importsClause)
      ...
      Case objInit           As ObjectCreationInitializerSyntax     : Quote_ObjectCreationInitializerSyntax(objInit)
      Case order             As OrderingSyntax
      Case paramList         As ParameterListSyntax
      Case param             As ParameterSyntax
      Case queryClause       As QueryClauseSyntax                   : Quote_QueryClauseSyntax(queryClause)
      ...
      Case structuredTrivia  As StructuredTriviaSyntax              : Quote_StructuredTriviaSyntax(structuredTrivia)
      Case tArgList          As TypeArgumentListSyntax
      Case tParamConstraint  As TypeParameterConstraintClauseSyntax : Quote_TypeParameterConstraintClauseSyntax(tParamConstraint)
...
#Region "Expression"
  ...
  Private Sub Quute_Argument(arg As ArgumentSyntax)
    Select Case arg
      ...
      Case Else
        Throw New NotSupportedException(String.Format("Expression not supported: {0}", arg))
...
#End Region
  ...
  Sub Quote_AsClause(thisclause As AsClauseSyntax)
    Select Case thisclause
      Case clause As AsNewClauseSyntax
      Case clause As SimpleAsClauseSyntax
      Case Else
        Throw New NotSupportedException(String.Format("AsClause not supported: {0}", thisclause))
    End Select
  End Sub


  Sub Quote_CaseClause(caseClause As CaseClauseSyntax)
    Select Case caseClause
      Case c As ElseCaseClauseSyntax
      Case c As RangeCaseClauseSyntax
      Case c As RelationalCaseClauseSyntax
      Case c As SimpleCaseClauseSyntax
      Case Else
        Throw New NotSupportedException(String.Format("CaseClauseSyntax not supported: {0}", caseClause))
    End Select
  End Sub

  Sub Quote_ConstrainClause(constriant As ConstraintSyntax)
    Select Case constriant
      Case c As SpecialConstraintSyntax
      Case c As TypeConstraintSyntax
      Case Else
        Throw New NotSupportedException(String.Format("ConstraintSyntax not supported: {0}", constriant))
    End Select
  End Sub

  Sub Quote_FieldInitializerSyntax(field As FieldInitializerSyntax )
    Select Case field
      Case c As InferredFieldInitializerSyntax
      Case c As NamedFieldInitializerSyntax
      Case Else
        Throw New NotSupportedException(String.Format("FieldInitializerSyntax not supported: {0}", field))
    End Select
  End Sub

  Sub Quote_ImportsClauseSyntax(import As ImportsClauseSyntax)
    Select Case import
      Case c As SimpleImportsClauseSyntax
      Case c As XmlNamespaceImportsClauseSyntax
      Case Else
        Throw New NotSupportedException(String.Format("ImportsClauseSyntax not supported: {0}", import))
    End Select
  End Sub

  Sub Quote_ObjectCreationInitializerSyntax(import As ObjectCreationInitializerSyntax)
    Select Case import
      Case c As ObjectCollectionInitializerSyntax
      Case c As ObjectMemberInitializerSyntax
      Case Else
        Throw New NotSupportedException(String.Format("ObjectCreationInitializerSyntax not supported: {0}", import))
    End Select
  End Sub

  Sub Quote_QueryClauseSyntax(import As QueryClauseSyntax)
    Select Case import
      Case c As AggregateClauseSyntax
      Case c As DistinctClauseSyntax
      Case c As FromClauseSyntax
      Case c As GroupByClauseSyntax
      Case c As JoinClauseSyntax
      Case c As LetClauseSyntax
      Case c As OrderByClauseSyntax
      Case c As PartitionClauseSyntax
      Case c As PartitionWhileClauseSyntax
      Case c As SelectClauseSyntax
      Case c As WhereClauseSyntax
      Case Else
        Throw New NotSupportedException(String.Format("QueryClauseSyntax not supported: {0}", import))
    End Select
  End Sub

  Sub Quote_StructuredTriviaSyntax (import As StructuredTriviaSyntax )
    Select Case import
      Case c As DirectiveTriviaSyntax
      Case c As DocumentationCommentTriviaSyntax
      Case c As SkippedTokensTriviaSyntax
      Case Else
        Throw New NotSupportedException(String.Format("StructuredTriviaSyntax  not supported: {0}", import))
    End Select
  End Sub

  Sub Quote_TypeParameterConstraintClauseSyntax (import As TypeParameterConstraintClauseSyntax )
    Select Case import
      Case c As TypeParameterMultipleConstraintClauseSyntax
      Case c As TypeParameterSingleConstraintClauseSyntax
      Case Else
        Throw New NotSupportedException(String.Format("TypeParameterConstraintClauseSyntax  not supported: {0}", import))
    End Select
  End Sub

  'Sub Quote_(import As )
  '  Select Case import
  '    Case c As SimpleImportsClauseSyntax
  '    Case c As XmlNamespaceImportsClauseSyntax
  '    Case Else
  '      Throw New NotSupportedException(String.Format(" not supported: {0}", import))
  '  End Select
  'End Sub
